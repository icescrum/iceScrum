        ...
        stateHelperProvider
            ...
            .state({
                ...
                resolve: {
                    ...
                    window: ['WindowService', 'project', function(WindowService, project) {
                        return WindowService.get('roadmap', project);
                    }],
                    roadmaps: ['releases', 'features', function(releases, features) {
                        return [{
                            id:1,
                            name:'Current',
                            readonly: true,
                            lastUpdated:'11:11:00 12/22/2017',
                            data: function(){
                                var data = [];
                                //releases
                                data.push({
                                ...
                                _.each(releases, function(release){
                                    data[0].periods.push(
                                        {id: "release-"+release.id, "starting_time": release.startDate, "ending_time": release.endDate, height: 20, color: "rgba(45, 140, 204, 0.3)", "details": "/planning/"+release.id+"/details", tooltip:release.name }
                                    );
                                    _.each(release.sprints, function(sprint){
                                        data[0].periods.push(
                                            {id: "sprint-"+sprint.id, "starting_time": sprint.startDate, "ending_time": sprint.endDate, height: 10, color: "rgba(45, 140, 204, 0.5)", "details": "/planning/"+release.id+"/sprint/"+sprint.id+"/details", tooltip:sprint.goal? sprint.goal : "Sprint "+ sprint.orderNumber }
                                        );
                                        if(sprint.deliveredVersion){
                                            data[0].events.push({id: "event-"+sprint.id, "starting_time": sprint.endDate, "ending_time": sprint.endDate, color: "rgb(45, 140, 204)", tooltip:sprint.deliveredVersion })
                                        }
                                    });
                                });
                                ...
                                data = _.union(data, [
                                    {
                                        class: "features",
                                        tooltip: features[0].name,
                                        color:"rgba(156, 162, 166, 1.0)",
                                        periods: [
                                            {id: features[0].id, "starting_time": format.parse("2017-09-10"), "ending_time": format.parse("2017-10-07"), "details": "/roadmap/feature/"+features[0].id, tooltip:features[0].name + " - 3 stories"},
                                            {id: features[0].id, "starting_time": format.parse("2017-11-14"), "ending_time": format.parse("2017-11-31"), "details": "/roadmap/feature/"+features[0].id, tooltip:features[0].name + " - 5 stories"}
                                        ],
                                        intervals: [
                                            {"starting_time": format.parse("2017-10-07"), "ending_time": format.parse("2017-11-14")}
                                        ],
                                        events: [
                                            {id: features[0].id, "starting_time": format.parse("2017-11-01"), "ending_time": format.parse("2017-11-01"), tooltip:"Component delivered by supplier"}
                                        ]
                                    },
                                    {
                                        class: "features",
                                        tooltip: features[1].id,
                                        color:"rgba(217, 26, 47, 1.0)",
                                        periods: [
                                            {id: features[1].id, "starting_time": format.parse("2017-09-24"), "ending_time": format.parse("2017-10-07"), "details": "/roadmap/feature/"+features[1].id, tooltip:features[1].name + " - 2 stories"},
                                            {id: features[1].id, "starting_time": format.parse("2017-10-22"), "ending_time": format.parse("2017-11-04"), "details": "/roadmap/feature/"+features[1].id, tooltip:features[1].name + " - 2 stories"}
                                        ],
                                        intervals: [
                                            {"starting_time": format.parse("2017-10-07"), "ending_time": format.parse("2017-10-22")}
                                        ]
                                    },
                                    {
                                        class: "features",
                                        tooltip: "Advertising",
                                        color:"rgba(186, 72, 199, 1.0)",
                                        periods: [
                                            {id: features[2].id, "starting_time": format.parse("2017-09-10"), "ending_time": format.parse("2017-10-21"), "details": "/roadmap/feature/"+features[2].id, tooltip:features[2].name + " - 6 stories"}
                                        ]
                                    },
                                    {
                                        class: "features",
                                        tooltip: features[1].name,
                                        color:"rgba(95, 191, 128, 1.0)",
                                        periods: [
                                            {id: features[3].id, "starting_time": format.parse("2017-10-22"), "ending_time": format.parse("2017-11-13"), "details": "/roadmap/feature/"+features[3].id, tooltip:features[3].name + " - 2 stories"}
                                        ]
                                    }
                                ]);
                                return data;
                ...
                children: [
                    ...
                    },
                    {
                        ...
                        url: "/:elementId",
                        resolve: {
                            roadmap: ['$stateParams', 'roadmaps', function($stateParams, roadmaps) {
                                return _.find(roadmaps, {code: $stateParams.elementId});
                            }]
                        }
                    },
                    {
                        ...
                        views: {
                            "details": {
                                templateUrl: 'roadmap.details.html'
                            }
                        }
                    },
                    {
                        ...
                        children:[
                            ...
                            getFeatureDetailsState('@roadmap')
                        ]
                    },
                    {
                        name: 'feature',
                        ...
                        views: {
                            "details": {
                                templateUrl: 'roadmap.feature.new.html',
