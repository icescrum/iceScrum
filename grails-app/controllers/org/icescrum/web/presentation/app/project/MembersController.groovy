...
class MembersController {
    ...
    def browseList = {
        ...
        def teams = request.admin ? Team.findAllByNameIlike(term, options) : Team.findAllByOwnerOrSM(user.username, options, term)
        def total = request.admin ? Team.countByNameIlike(term, [cache:true]) : Team.countByOwnerOrSM(user.username, [cache:true], term)
        def results = []
        teams?.each {
