    ...
    changeSet(author: "vbarrier", id: "add_default_timeboxNote_template") {
        grailsChange {
            change {
                ...
                projects.each{
                    query += "('<h2>HTML Release Note Template</h2><ul>', '<h1> My HTML release Note </h1>', '$configsDataHtml', $it.id, 1),('Markdown Release Note Template', '# My Markdown release Note', '$configsDataMarkdown', $it.id, 1)"
                    if(it != projects.last()){
                        query += ","
                    }
                    count++
