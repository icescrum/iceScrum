            ...
            <div class="panel-box">
                ...
                <div class="panel-box-content">
                    <g:if test="${team.description}">
                        <wikitext:renderHtml
                                markup="Textile">${is.truncated(value: team.description, size: 1000, encodedHTML: false)}</wikitext:renderHtml>
                    </g:if>
                    <g:else>
                            ...
                            <li ${(storyActivities.size() == (i + 1)) ? 'class="last"' : ''}>
                                <div class="news-item news-${a.code}">
                                    <p><is:scrumLink controller="user" action='profile'
                                        ...
                                        <g:link class="scrum-link" action="idURL" controller="backlogElement"
                                                id="${a.cachedId}">${a.cachedLabel.encodeAsHTML()}</g:link></p>

                                    <p><g:formatDate date="${a.dateCreated}"
                                                     formatName="is.date.format.short.time"/></p>
                                </div>
                            </li>
                        </g:each>
                    </ul>
