...
<jq:jquery>
    ...
    <is:editable rendered="${sprint.state != Sprint.STATE_DONE && request.inProduct}"
                 ...
                 cancel="jQuery(original).next().show();"
                 ajaxoptions = "{dataType:'json'}"
                 callback="jQuery(this).next().show(); jQuery(this).html(jQuery.icescrum.formattedTaskEstimation(value.estimation, true)); jQuery.icescrum.sprint.updateRemaining(); if(value.state == ${Task.STATE_DONE}){ jQuery.event.trigger('update_task',value); }"
                 params="[product:params.product]"
