    ...
    <div class="colset-2-80 clearfix">
        <div class="col1">
            ...
            <is:panel id="panel-infos">
                ...
                <is:panelContext>
                    <is:panelLine
                            legend="${message(code:'is.user.firstname')}">${user.firstName?.encodeAsHTML()}</is:panelLine>
                    <is:panelLine
                            legend="${message(code:'is.user.lastname')}">${user.lastName?.encodeAsHTML()}</is:panelLine>
                    <is:panelLine
                            legend="${message(code:'is.user.username')}">${user.username.encodeAsHTML()}</is:panelLine>
                    <is:panelLine
                            legend="${message(code:'is.user.preferences.activity')}">${user.preferences.activity?.encodeAsHTML()}</is:panelLine>
                    <is:panelLine legend="${message(code:'is.user.email')}"><a
                            href="mailto:${user.email}">${user.email.encodeAsHTML()}</a></is:panelLine>
            ...
            <div id="activities-wrapper">
                <g:render template="window/activities"
                          model="[activities:activities, user:user, tasks:tasks, stories:stories]"/>
            </div>
