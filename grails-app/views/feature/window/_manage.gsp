...
<g:form action="save" method="post" name="${id}-form" class="box-form box-form-250 box-form-200-legend" tabindex="1">
  ...
  <is:fieldset title="is.ui.feature.feature.properties.title">
    ...
    <is:fieldSelect for="feature.rank" label="is.feature.rank">
      <is:select container="#window-content-feature" width="100" maxHeight="100" styleSelect="dropdown" from="${rankList}" name="feature.rank" value="${feature?.rank?:rankList.last()}"/>
    </is:fieldSelect>

    <is:fieldSelect for="feature.value" label="is.feature.value">
      <is:select container="#window-content-${id}" width="100" maxHeight="100" styleSelect="dropdown" from="${valuesList}" name="feature.value" value="${feature?.value}"/>
    </is:fieldSelect>

    <is:fieldSelect for="feature.type" label="is.feature.type">
      <is:select container="#window-content-feature" width="100" maxHeight="100" styleSelect="dropdown" from="${typesNames}" keys="${typesId}" name="feature.type" value="${feature?.type}"/>
    </is:fieldSelect>


    <is:fieldSelect for="featureColor" label="is.feature.color" noborder="true">
      <is:select class="featureColor" container="#window-content-feature" width="100" maxHeight="100" styleSelect="dropdown" from="${colorsLabels}" keys="${colorsKeys}" name="feature.color" value="${feature?.color}" onchange="\$('#postit-ipsum-1').find('.postit-layout').removeClass().addClass('postit-layout postit-'+this.value);"/>
