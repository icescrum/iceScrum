...
<is:wizard next="is.dialog.wizard.next" cancel="is.button.cancel" previous="is.dialog.wizard.previous" submit="is.dialog.wizard.submit" id="project-wizard" controller="project" before="\$('#choose-select-is-team-teams').empty();\$('#choose-select-is-team-members').empty();" onSuccess="\$('#dialog').dialog('close');" update="dialog" action="save">
  ...
  <is:fieldset title="is.dialog.wizard.section.project" description="is.dialog.wizard.section.project.description">
    ...
    <is:fieldDatePicker noborder="${privateOption?'true':''}" for="productstartDate" label="is.dialog.wizard.project.startDate">
      <is:datePicker id="productstartDate" defaultDate="${product.startDate}" onSelect="jQuery.icescrum.updateWizardDate(this);" name="product.startDate" mode="read-input" changeMonth="true" changeYear="true"/>
    </is:fieldDatePicker>
  ...
  <is:fieldset title="is.dialog.wizard.section.team" description="is.dialog.wizard.section.team.description">
    ...
    <is:fieldRadio for="team.newTeam" label="is.product.team.new">
      <is:radio id="team.newTeam" name="team.newTeam" value="${true}" onClick="if(this.value == 1){\$('#newTeamFieldSet').show();\$('#existingTeamFieldSet').hide();}else{\$('#newTeamFieldSet').hide();\$('#existingTeamFieldSet').show();}"/>
    </is:fieldRadio>
    <span id="newTeamFieldSet">
      ...
      <is:fieldArea for="team.description" optional="true" label="is.team.description">
        <is:area rich="[preview:true,width:335]"
               id="teamdescription"
  ...
  <is:fieldset title="is.dialog.wizard.section.options" description="is.dialog.wizard.section.options.description">
    <is:fieldSelect for="product.planningPokerGameType" label="is.product.preferences.planification.estimationSuite">
      <is:select from="${estimationSuitSelect.values().asList()}" keys="${estimationSuitSelect.keySet().asList()}" width="240" maxHeight="100" styleSelect="dropdown" name="product.planningPokerGameType" id="product.planningPokerGameType" value="${product.planningPokerGameType}"/>
    </is:fieldSelect>
    ...
    <is:fieldRadio for="product.preferences.autoCreateTaskOnEmptyStory" label="is.product.preferences.sprint.autoCreateTaskOnEmptyStory">
      <is:radio id="product.preferences.autoCreateTaskOnEmptyStory" name="product.preferences.autoCreateTaskOnEmptyStory" value="${product.preferences.autoCreateTaskOnEmptyStory}"/>
    </is:fieldRadio>
    <is:fieldRadio for="product.preferences.assignOnCreateTask" label="is.product.preferences.sprint.assignOnCreateTask">
      <is:radio id="product.preferences.assignOnCreateTask" name="product.preferences.assignOnCreateTask" value="${product.preferences.assignOnCreateTask}"/>
    </is:fieldRadio>
    <is:fieldRadio for="product.preferences.assignOnBeginTask" label="is.product.preferences.sprint.assignOnBeginTask">
      <is:radio id="product.preferences.assignOnBeginTask" name="product.preferences.assignOnBeginTask" value="${product.preferences.assignOnBeginTask}"/>
    </is:fieldRadio>
    <is:fieldRadio for="product.preferences.displayRecurrentTasks" label="is.product.preferences.sprint.displayRecurrentTasks">
      <is:radio id="product.preferences.displayRecurrentTasks" name="product.preferences.displayRecurrentTasks" value="${product.preferences.displayRecurrentTasks}"/>
    </is:fieldRadio>
  ...
  <is:fieldset title="is.dialog.wizard.section.starting" description="is.dialog.wizard.section.starting.description">
    <is:fieldDatePicker for="productendDate" label="is.dialog.wizard.project.endDate">
      <is:datePicker id="productendDate" name="product.endDate" defaultDate="${product.endDate}" minDate="${product.startDate}" onSelect="
    ...
    <is:fieldDatePicker for="firstSprint" label="is.dialog.wizard.firstSprint">
      <is:datePicker id="firstSprint" name="firstSprint" defaultDate="${product.startDate}" minDate="${product.startDate}" maxDate="${product.endDate - 1}" mode="read-input" changeMonth="true" changeYear="true"/>
    </is:fieldDatePicker>
    <is:fieldInput for="productpreferencesestimatedSprintsDuration" label="is.product.preferences.planification.estimatedSprintsDuration" noborder="true">
      <is:input id="productpreferencesestimatedSprintsDuration" name="product.preferences.estimatedSprintsDuration"  typed="[type:'numeric']" value="${product.preferences.estimatedSprintsDuration}"/>
    </is:fieldInput>
  </is:fieldset>
