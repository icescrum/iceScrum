...
<is:modal title="${message(code: 'is.dialog.importProject.choose.title')}" footer="${false}" class="import">
    <div flow-drop
        ...
        <div ng-hide="$flow.files.length" class="form-text text-center mt-4">
            <g:message code="is.dialog.importProject.choose.description"/>
        </div>
        <div ng-hide="$flow.files.length" class="text-center mt-4 mb-4">
            <button class="btn btn-primary" flow-btn>${message(code: 'is.dialog.importProject.choose.file')}</button>
        </div>
